...
"NL99KNAB9999999999";"27-03-2020";"EUR";"C";"5,00";"50022270";"Gert Janssen";"28-03-2020";"Ontvangen betaling";"Omschrijving 2";"";"";"";"";"C0C27PGFM28ERA34";"29-03-2020";
        ...
        self.assertEqual(
            statement.start_date, datetime.strptime("28-03-2020", parser.date_format)
        )

        self.assertIsNone(statement.end_balance)
        self.assertEqual(
            statement.end_date, datetime.strptime("30-03-2020", parser.date_format)
        )

        # Amount of 0 is skipped
        self.assertEqual(len(statement.lines), 2)
        self.assertEqual(
            statement.lines[0].date, datetime.strptime("28-03-2020", parser.date_format)
        )
        self.assertEqual(
            statement.lines[0].date_user,
            datetime.strptime("26-03-2020", parser.date_format),
        )
        ...
        self.assertEqual(
            statement.lines[1].date, datetime.strptime("29-03-2020", parser.date_format)
        )
        self.assertEqual(
            statement.lines[1].date_user,
            datetime.strptime("27-03-2020", parser.date_format),
        )
    ...
    def test_ok(self):
        ...
        self.assertIsNone(statement.start_balance)
        self.assertEqual(
            statement.start_date, datetime.strptime("07-01-2019", parser.date_format)
        )
    ...
    def test_no_header2(self):
        ...
        text_filename = os.path.join(
            here, "samples", "Knab_transactieoverzicht_no_header2.csv"
        )
        parser = Plugin(None, None).get_parser(text_filename)
    ...
    def test_no_lines(self):
        here = os.path.dirname(__file__)
        text_filename = os.path.join(
            here, "samples", "Knab_transactieoverzicht_no_lines.csv"
        )
        parser = Plugin(None, None).get_parser(text_filename)
